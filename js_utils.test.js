  ...
  describe("overload", function() {
    ...
    describe("matchers (single block)", function() {
      ...
      it("nullTypeMatcher", function() {
        var f;
        f = JSUtils.overload(JSUtils.overload.signature([null], JSUtils.overload.matchers.nullTypeMatcher), function(arg) {
          return arg;
        });
        expect(f(null)).toBe(null);
        expect(f(void 0)).toBe(void 0);
      ...
      it("arrayTypeMatcher", function() {
        var f;
        f = JSUtils.overload(JSUtils.overload.signature([[String]], JSUtils.overload.matchers.arrayTypeMatcher), JSUtils.overload.signature([[Number]], JSUtils.overload.matchers.arrayTypeMatcher), function(arr) {
          return arr.reduce(function(prev, curr) {
            return prev + curr;
          });
        });
        expect(f([1, 2, 3])).toBe(6);
        expect(f(["1", "2", "3"])).toBe("123");
        expect(function() {
          return f(["1", 2]);
        }).toThrow();
        expect(function() {
      ...
      return it("anyTypeMatcher", function() {
        var a, f;
        f = JSUtils.overload(JSUtils.overload.signature([JSUtils.overload.ANY], JSUtils.overload.matchers.anyTypeMatcher), function(arg) {
          return arg;
        });
        expect(f(false)).toBe(false);
        expect(f({})).toEqual({});
    ...
    describe("more realistic use cases (multiple, blocks, multiple matchers)", function() {
      ...
      return it("optional parameters", function() {
        ...
        Item = (function() {
          ...
          ctor = JSUtils.overload([String], [String, Number], function(str, optNum) {
            this.list = [];
            this.str = str;
    ...
    describe("nativesPrototyping", function() {
      describe("Math", function() {
        it("isNum", function() {
          ...
          expect(Math.isNum(2e308)).toBe(false);
          expect(Math.isNum(-2e308)).toBe(false);
        ...
        it("sign", function() {
          ...
          return expect(Math.sign(2e308)).toBe(void 0);
        });
    ...
    describe("Tree", function() {
      ...
      describe("modifying a tree", function() {
        ...
        it("addChild (== appendChild)", function() {
          expect(this.tree.getDescendants().length).toBe(4);
          ...
          expect(this.tree.getDescendants().length).toBe(5);
          this.tree.addChild({
            prop: "asdf"
          }, 1);
          expect(this.tree.children.length).toBe(5);
          expect(this.tree.children[1].prop).toBe("asdf");
          expect(this.tree.children[4].newNode).toBe(true);
          return expect(this.tree.getDescendants().length).toBe(6);
        });
        it("addChildren (== appendChildren)", function() {
          ...
          expect(this.tree.children.length).toBe(5);
          expect(this.tree.children[0].name).toBe("child1");
          ...
          expect(this.tree.children[4].name).toBe("child3");
          return expect(this.tree.getDescendants().length).toBe(6);
        });
        it("setChildren", function() {
          ...
          expect(this.tree.children[1].name).toBe("new child 2");
          return expect(this.tree.getDescendants().length).toBe(2);
        });
